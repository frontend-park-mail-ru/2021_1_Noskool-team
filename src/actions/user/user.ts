...
import { PROFILE, CHANGE_PROFILE, CHANGE_USER_PHOTE, GET_CSRF } from './user.constants';
...
export const getUser = async (): Promise<UserProfile | undefined> => {
    ...
    if (response.status === 401) {
        ...
        const getCsrf = await getcsrf(GET_CSRF);
...
export const changeUser = async (body: UserChangeData) => {
    ...
    if (response.status === 401) {
        ...
        const getCsrf = await getcsrf(GET_CSRF);
        if (getCsrf.status === 200) {
            response = await post(CHANGE_PROFILE, body);
            if (response.status !== 200) {
                redirectTo(LINKS.auth);
                return new Promise(() => {});
            }
